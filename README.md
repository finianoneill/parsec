...
<p align="center">
  ...
  <img alt="426 tests" src="https://img.shields.io/badge/tests-426%20passing-16a34a?style=flat-square">
...
parsec ❯ /refresh <id>       # re-run a session's question now, diff against it
...
Two decision points can pause a live run for your input, both built on the same replay-safe primitive (a journaled proposal blocks for one steering reply, recorded gate-tagged and turn-indexed, so gated sessions replay byte-identically):

- `--brief-gate` pauses at the proposed research brief — `approve` dispatches, `edit` opens it in `$EDITOR`, anything else requests changes.
- `--cost-gate 0.5` pauses **once** when spend crosses that fraction of the USD cap at a dispatch boundary — `approve` continues; any other reply stops dispatching, remaining subquestions resolve as declined, and the writer wraps up with what was gathered.
...
| `parsec refresh <session>` | Re-run a recorded session's query as a new session seeded by its brief and coverage ledger: stable-evidence subquestions carry forward (identical node IDs, no dispatch), volatile/slow or unresolved ones re-research live, then the claim-level diff against the original is emitted. `--all` re-researches everything. Exits 3 on material change |
...
| `parsec watch <session>` | Scheduled refresh: re-check a question on an interval (`--every 6h`, `--rounds N`; one round without `--every` — cron-friendly), refreshing the newest session in the chain each round, reporting only material diffs, and labeling the prior run's claims held/overturned as `(credence, outcome)` pairs appended to `watch-labels.json` for `parsec calibrate`. Exits 3 when any round was material |
...
| `parsec sessions list` / `show <session>` | Sessions, spend (totals and per stream — which subquestion spent what), and the coverage ledger |
...
Budget caps default deliberately low and are debited in real time to a per-actor, per-stream ledger: `$0.50` / `200k tokens` / `300s` / `12 turns`. Raise them explicitly (`--cost-gate` adds a mid-run approval checkpoint instead of a hard stop). Fan-out research is a token-spend strategy — parsec makes the spend visible per node of the graph it bought, and per subquestion after the fact.

**Failures are part of the trajectory.** Model-call errors classify into an actionable taxonomy (throttled / overloaded / context-overflow / transient / fatal); retryable ones back off with bounded, jitter-free delays journaled per attempt as `LLM_RETRY` events, context overflows trigger compaction-and-retry instead of a dead subagent, and all of it replays: the replay adapter serves recorded failed attempts before a call's final outcome, so a throttled-and-recovered run reproduces its exact retry sequence. Sessions also record the parsec version that produced them — `replay` and `fork` warn on mismatch, so a divergence reads as version skew rather than corruption.

**Prompt caching** defaults to full-prefix placement on new runs (`--cache-strategy full`): breakpoints on the system block, the tool-schema array, and a rolling marker on the final message block, so each request's append-only prefix re-bills at 0.1× on the next call. Placement is recorded per session; pre-existing recordings keep their original wire shape and still replay.
...
```sh
uv run pytest                                        # 426 tests, no network/keys
...
- [x] **M14.2 — `parsec refresh <session>`**: re-run a persisted query as a new session seeded by the prior brief and coverage ledger — no re-planning: the parent's recorded scope, effort, and subquestions are reused verbatim (zero decomposition calls), and mutability classes govern what gets re-researched. An answered subquestion whose evidence is all `stable` **carries forward**: its premises, findings, and source spans are re-recorded into the new session, where content-derived node IDs preserve cross-session identity — the closing diff matches them by id. Anything volatile or slow (the world may have moved), anything left partial/blocked/dropped, or everything under `--all`, re-researches **live**: the refresh fetches in record mode, so cached bytes never stand in for the web it exists to re-observe. The parent's replay survives that on purpose — replay and fork now *pin* fetches to the doc hashes the session's own events recorded, instead of the shared per-URL cache row the refresh advances. The refreshed session is itself a first-class recording: `refresh_of`/`--all` freeze into its config, the seed is a pure function of the immutable parent, and it replays byte-identically (T4). The run ends by emitting `parsec diff parent refreshed` — exit 3 on material change, so it scripts. Gates are stripped (the brief was approved in the parent; refresh runs headless — what M14.3's scheduler needs). *Exit tests green: a stable subquestion carries forward with identical node IDs and no dispatch while a volatile one re-researches against changed bytes, the run's only orchestrator model call is the writer, and both the refreshed session AND the parent still replay byte-identically after the re-fetch; `--all` provably re-dispatches everything.*
- [x] **M14.3 — `parsec watch <session>`**: scheduled refresh that reports only material diffs — and mechanically labels the prior run's claims as `(credence, outcome)` pairs, turning `parsec calibrate` into a flywheel: time is the oracle research otherwise lacks (T2). A watch is a **chain of refreshes**: each round refreshes the *newest* session in the chain (so stable evidence keeps carrying forward and the diff reads "what changed since the last look"), sleeps `--every 6h` between rounds, and stops at `--rounds N` or Ctrl-C; without `--every` it runs one round and exits, which is what a cron entry wants. **Material-only reporting**: a quiet round is one dim line (or a JSON line carrying counts and `diff: null`); a material round renders the full claim-level diff; exit 3 if any round was material. **Mechanical outcome labels**: the prior observation's claims are scored by what time did to them — held / strengthened / weakened read as *held* (1: the claim still stands, whatever its credence did), superseded reads as *overturned* (0: newer evidence replaced its support), retracted reads as overturned only when the refresh finished `done` (full coverage, so absence is a finding) and is skipped as ambiguous when it was partial; `new` claims have nothing prior to score. Each label pairs the claim's credence *as the prior run recorded it* with the outcome, appended to `<data-dir>/watch-labels.json` in the `{credence, label}` shape `parsec calibrate` already reads — a standing watch grades its own past predictions without a human in the loop. The watch is orchestration, not a recording: every round is a first-class refresh session that replays on its own (T4); the labels file is its durable output. *Exit tests green: two scheduled rounds chain off the newest session with exactly one schedule sleep between them; a refresh that drops a claim reports as material with the retraction while an identical follow-up is quiet with its diff withheld; the parent's claims label held/overturned with their recorded credences and the file round-trips through `parsec calibrate`'s reader; the chain's tail replays byte-identically; the CLI emits one JSON line per round plus a summary and exits 3.*
