...
<p align="center">
  ...
  <img alt="286 tests" src="https://img.shields.io/badge/tests-286%20passing-16a34a?style=flat-square">
...
|---|---|
| `parsec ask "…"` | Run a research query (`--live` for the progress view, `--parallel N` for concurrent subagents (≤5), `--max-usd`/`--max-tokens`/`--max-turns`/`--max-gap-rounds` for budgets, `--cache-mode record\|replay\|live-prefer-cache`) |
...
```sh
uv run pytest                                        # 286 tests, no network/keys
