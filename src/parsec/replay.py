...
async def run_replay(
    ...
    for ev in event_log.read(original_session_id):
        if ev.event_type == EventType.FETCH_PERFORMED:
            # Pin each fetch to the byte version THIS session recorded, in
            # per-stream fetch order: the shared cache_index row advances
            # whenever a later session re-fetches the URL (`parsec refresh`
            # does, on purpose), and the recording itself may have fetched
            # one URL twice and seen different bytes — an unpinned replay
            # would read bytes the recording never saw (T4).
